...
typedef struct ten_go_ten_env_tester_send_video_frame_ctx_t {
  ten_shared_ptr_t *c_video_frame;
  ten_go_handle_t handler_id;
} ten_go_ten_env_tester_send_video_frame_ctx_t;

static ten_go_ten_env_tester_send_video_frame_ctx_t *
ten_go_ten_env_tester_send_video_frame_ctx_create(
    ten_shared_ptr_t *c_video_frame, ten_go_handle_t handler_id) {
  TEN_ASSERT(c_video_frame, "Invalid argument.");

  ten_go_ten_env_tester_send_video_frame_ctx_t *ctx =
      TEN_MALLOC(sizeof(ten_go_ten_env_tester_send_video_frame_ctx_t));
...
static void ten_go_ten_env_tester_send_video_frame_ctx_destroy(
    ten_go_ten_env_tester_send_video_frame_ctx_t *ctx) {
...
static void ten_env_tester_proxy_notify_send_video_frame(
  ...
  ten_go_ten_env_tester_send_video_frame_ctx_t *ctx = user_video_frame;
  ...
  if (ctx->handler_id == TEN_GO_NO_RESPONSE_HANDLER) {
    res = ten_env_tester_send_video_frame(ten_env_tester, ctx->c_video_frame,
                                          NULL, NULL, &err);
  } else {
    ten_go_callback_ctx_t *callback_ctx =
        ten_go_callback_ctx_create(ctx->handler_id);
    res = ten_env_tester_send_video_frame(ten_env_tester, ctx->c_video_frame,
                                          proxy_handle_video_frame_error,
                                          callback_ctx, &err);
    if (!res) {
  ...
  ten_go_ten_env_tester_send_video_frame_ctx_destroy(ctx);
}
...
ten_go_error_t ten_go_ten_env_tester_send_video_frame(
  ...
  TEN_ASSERT(video_frame && ten_go_msg_check_integrity(video_frame),
             "Should not happen.");
  TEN_ASSERT(ten_go_msg_c_msg(video_frame), "Should not happen.");
  ...
  if (!self->c_ten_env_tester_proxy) {
    ten_go_error_set(
        ...
        "ten_env_tester.send_video_frame() failed because the TEN is "
        "closed.");
    return cgo_error;
  }
  ...
  if (!ten_env_tester_proxy_notify(self->c_ten_env_tester_proxy,
                                   ten_env_tester_proxy_notify_send_video_frame,
                                   ctx, &err)) {
    ten_go_ten_env_tester_send_video_frame_ctx_destroy(ctx);
    ten_go_error_set_from_error(&cgo_error, &err);
  }
