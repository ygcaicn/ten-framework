...
//
// The final application executable will link ten_runtime_go.so, if asan is
// enabled in compiling ten_runtime_go.so, the C compiler must be same as the
// compiler used to compile ten_runtime_go.so.
//
...
// compiled with asan. However, this is the job of the tman when installing
...
func LoadExtensionPackageModule(
	...
	// Check if the folder contains a go.mod to determine if it is a GO
	// extension.
	modFile := path.Join(location, "go.mod")
	if !IsFilePresent(modFile) {
		if verbose {
			log.Printf("%s is not a GO extension, no go.mod", location)
		}
		return nil, nil
	}
...
func (ab *AppBuilder) autoDetectExtensionPackageModules() error {
	...
	if !IsDirPresent(extBaseDir) {
		if ab.options.Verbose {
			log.Println(
				"The base directory [ten_packages/extension] is absent, no extensions.",
			)
		}
...
func LoadManifest(pkgDir string) (*TenPackageManifest, error) {
	...
	if mf, err := os.ReadFile(manifest); err != nil {
		...
		var manifest *TenPackageManifest
...
func (pm *TenPackageManifest) IsApp() error {
	if pm.Type != "app" {
		return fmt.Errorf(
...
func (pm *TenPackageManifest) IsExtension() error {
