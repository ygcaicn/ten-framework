...
static void ten_env_tester_proxy_notify_on_stop_done(
  ...
  bool rc = ten_env_tester_on_stop_done(ten_env_tester, &err);
...
napi_value ten_nodejs_ten_env_tester_on_stop_done(napi_env env,
  ...
  bool rc = ten_env_tester_proxy_notify(
      ...
      ten_env_tester_proxy_notify_on_stop_done, NULL, &err);
  if (!rc) {
