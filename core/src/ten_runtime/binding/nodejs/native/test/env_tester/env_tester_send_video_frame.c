...
typedef struct ten_env_tester_notify_send_video_frame_ctx_t {
  ten_shared_ptr_t *c_video_frame;
  ten_nodejs_tsfn_t *js_cb;
} ten_env_tester_notify_send_video_frame_ctx_t;
...
static ten_env_tester_notify_send_video_frame_ctx_t *
ten_env_tester_notify_send_video_frame_ctx_create(
    ten_shared_ptr_t *c_video_frame, ten_nodejs_tsfn_t *js_cb) {
  ten_env_tester_notify_send_video_frame_ctx_t *ctx =
      TEN_MALLOC(sizeof(ten_env_tester_notify_send_video_frame_ctx_t));
...
static void ten_env_tester_notify_send_video_frame_ctx_destroy(
    ten_env_tester_notify_send_video_frame_ctx_t *ctx) {
...
static void proxy_send_video_frame_callback(
  ...
  ten_env_tester_notify_send_video_frame_ctx_destroy(ctx);
}

static void ten_env_tester_proxy_notify_send_video_frame(
    ten_env_tester_t *ten_env_tester, void *user_video_frame) {
  ...
  bool rc = ten_env_tester_send_video_frame(ten_env_tester, ctx->c_video_frame,
                                            proxy_send_video_frame_callback,
                                            ctx, &err);
  if (!rc) {
    proxy_send_video_frame_callback(ten_env_tester, NULL, ctx, &err);
  }

  ten_error_deinit(&err);
}

napi_value ten_nodejs_ten_env_tester_send_video_frame(napi_env env,
  ...
  if (ten_env_tester_bridge->c_ten_env_tester_proxy == NULL) {
    ...
    ten_error_set(&err, TEN_ERROR_CODE_TEN_IS_CLOSED,
                  "ten_env_tester.send_video_frame() failed because "
  ...
  ten_nodejs_tsfn_t *cb_tsfn = ten_nodejs_tsfn_create(
      env, "[TSFN] TenEnvTester::send_video_frame callback", args[2],
      tsfn_proxy_send_video_frame_callback);
  RETURN_UNDEFINED_IF_NAPI_FAIL(cb_tsfn, "Failed to create TSFN");

  ten_error_t err;
  TEN_ERROR_INIT(err);

  ten_env_tester_notify_send_video_frame_ctx_t *notify_info =
      ten_env_tester_notify_send_video_frame_ctx_create(
  ...
  bool rc = ten_env_tester_proxy_notify(
      ...
      ten_env_tester_proxy_notify_send_video_frame, notify_info, &err);
  if (!rc) {
    napi_value js_error = ten_nodejs_error_wrap(env, &err);
    RETURN_UNDEFINED_IF_NAPI_FAIL(js_error, "Failed to create JS error");
    ...
    ten_env_tester_notify_send_video_frame_ctx_destroy(notify_info);
    ten_error_deinit(&err);
    return js_error;
  }
