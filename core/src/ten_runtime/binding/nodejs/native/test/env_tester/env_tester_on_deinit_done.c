...
static void ten_env_tester_proxy_notify_on_deinit_done(
  ...
  ten_env_tester_proxy_t *env_tester_proxy = user_data;
  TEN_ASSERT(env_tester_proxy, "Should not happen.");

  bool rc = ten_env_tester_proxy_release(env_tester_proxy, &err);
  TEN_ASSERT(rc, "Should not happen.");

  rc = ten_env_tester_on_deinit_done(ten_env_tester, &err);
  TEN_ASSERT(rc, "Should not happen.");

  ten_error_deinit(&err);
}

napi_value ten_nodejs_ten_env_tester_on_deinit_done(napi_env env,
  ...
  bool rc = ten_env_tester_proxy_notify(
      ...
      ten_env_tester_proxy_notify_on_deinit_done,
      ten_env_tester_bridge->c_ten_env_tester_proxy, &err);
  if (!rc) {
  ...
  // Reset the c_ten_env_tester_proxy and c_ten_env_tester to NULL.
  ten_env_tester_bridge->c_ten_env_tester_proxy = NULL;
  ten_env_tester_bridge->c_ten_env_tester = NULL;

  // Release the reference to the JS ten_env_tester object.
  uint32_t js_ten_env_tester_ref_count = 0;
  status =
      napi_reference_unref(env, ten_env_tester_bridge->bridge.js_instance_ref,
