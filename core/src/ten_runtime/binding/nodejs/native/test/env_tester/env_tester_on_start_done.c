...
static void ten_env_tester_proxy_notify_on_start_done(
  ...
  bool rc = ten_env_tester_on_start_done(ten_env_tester, &err);
  TEN_ASSERT(rc, "Should not happen.");

  ten_error_deinit(&err);
}

napi_value ten_nodejs_ten_env_tester_on_start_done(napi_env env,
  ...
  bool rc = ten_env_tester_proxy_notify(
      ...
      ten_env_tester_proxy_notify_on_start_done, NULL, &err);
  if (!rc) {
