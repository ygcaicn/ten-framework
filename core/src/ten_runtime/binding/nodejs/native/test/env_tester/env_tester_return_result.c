...
typedef struct ten_env_tester_notify_return_result_ctx_t {
  ten_shared_ptr_t *c_cmd_result;
  ten_nodejs_tsfn_t *js_cb;
} ten_env_tester_notify_return_result_ctx_t;

typedef struct ten_nodejs_return_result_callback_call_ctx_t {
  ten_nodejs_tsfn_t *js_cb;
  ten_error_t *error;
} ten_nodejs_return_result_callback_call_ctx_t;

static ten_env_tester_notify_return_result_ctx_t *
ten_env_tester_notify_return_result_ctx_create(ten_shared_ptr_t *c_cmd_result,
                                               ten_nodejs_tsfn_t *js_cb) {
  ten_env_tester_notify_return_result_ctx_t *ctx =
      TEN_MALLOC(sizeof(ten_env_tester_notify_return_result_ctx_t));
...
static void ten_env_tester_notify_return_result_ctx_destroy(
    ten_env_tester_notify_return_result_ctx_t *ctx) {
...
static ten_nodejs_return_result_callback_call_ctx_t *
ten_nodejs_return_result_callback_call_ctx_create(ten_nodejs_tsfn_t *js_cb,
                                                  ten_error_t *error) {
  ten_nodejs_return_result_callback_call_ctx_t *ctx =
      TEN_MALLOC(sizeof(ten_nodejs_return_result_callback_call_ctx_t));
  TEN_ASSERT(ctx, "Failed to allocate memory.");
...
static void ten_nodejs_return_result_callback_call_ctx_destroy(
    ten_nodejs_return_result_callback_call_ctx_t *ctx) {
  TEN_ASSERT(ctx, "Invalid argument.");
...
static void proxy_return_result_error_callback(ten_env_tester_t *self,
                                               ten_shared_ptr_t *c_cmd_result,
                                               void *user_data,
                                               ten_error_t *err) {
  TEN_ASSERT(self, "Should not happen.");
  TEN_ASSERT(ten_env_tester_check_integrity(self, true), "Should not happen.");

  ten_env_tester_notify_return_result_ctx_t *ctx = user_data;
  TEN_ASSERT(ctx, "Should not happen.");
  ...
  ten_nodejs_return_result_callback_call_ctx_t *call_info =
      ten_nodejs_return_result_callback_call_ctx_create(js_cb, cloned_error);
  TEN_ASSERT(call_info, "Should not happen.");

  bool rc = ten_nodejs_tsfn_invoke(js_cb, call_info);
  TEN_ASSERT(rc, "Should not happen.");

  ten_env_tester_notify_return_result_ctx_destroy(ctx);
}
...
static void ten_env_tester_proxy_notify_return_result(
  ...
  bool rc = ten_env_tester_return_result(ten_env_tester, ctx->c_cmd_result,
                                         proxy_return_result_error_callback,
                                         ctx, &err);
  if (!rc) {
    proxy_return_result_error_callback(ten_env_tester, NULL, ctx, &err);
  }

  ten_error_deinit(&err);
}

napi_value ten_nodejs_ten_env_tester_return_result(napi_env env,
  ...
  if (ten_env_tester_bridge->c_ten_env_tester_proxy == NULL) {
    ...
    ten_error_set(&err, TEN_ERROR_CODE_TEN_IS_CLOSED,
                  "ten_env_tester.return_result() failed because "
  ...
  ten_nodejs_tsfn_t *cb_tsfn =
      ten_nodejs_tsfn_create(env, "[TSFN] TenEnvTester::return_result callback",
  ...
  ten_env_tester_notify_return_result_ctx_t *notify_info =
      ten_env_tester_notify_return_result_ctx_create(
  ...
  bool rc = ten_env_tester_proxy_notify(
      ...
      ten_env_tester_proxy_notify_return_result, notify_info, &err);
  if (!rc) {
    napi_value js_error = ten_nodejs_error_wrap(env, &err);
    RETURN_UNDEFINED_IF_NAPI_FAIL(js_error, "Failed to create JS error");
    ...
    ten_env_tester_notify_return_result_ctx_destroy(notify_info);
    ten_error_deinit(&err);
    return js_error;
  }
