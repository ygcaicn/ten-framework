...
def analyze_resolve_result(
    ext_root_folder: str, expected_json_file_name: str
) -> None:
    with open(
        os.path.join(ext_root_folder, expected_json_file_name),
        "r",
        encoding="utf-8",
        ...
        ten_packages_root = os.path.join(
            ext_root_folder, ".ten", "app", "ten_packages"
        )
...
def __test_tman_install(prod: bool):
    ...
    mock_extension_with_dev_deps_path = os.path.join(
        base_path, "mock_extension_with_dev_deps"
    )
    os.chdir(mock_extension_with_dev_deps_path)

    # If the .ten folder exists, remove it.
    ten_packages_folder = os.path.join(
        mock_extension_with_dev_deps_path, ".ten"
    )
    ...
    analyze_resolve_result(
        mock_extension_with_dev_deps_path, expected_json_file_name
    )
