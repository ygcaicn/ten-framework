...
def test_go_app_python():
    ...
    if sys.platform == "win32":
        print("test_go_app_python doesn't support win32")
        assert False
    elif sys.platform == "darwin":
        # client depends on some libraries in the TEN app.
        my_env["DYLD_LIBRARY_PATH"] = os.path.join(
            base_path, "go_app_python_app/lib"
        )
    else:
        # client depends on some libraries in the TEN app.
    ...
    bootstrap_cmd = os.path.join(base_path, "go_app_python_app/bin/bootstrap")
    ...
    server_cmd = os.path.join(base_path, "go_app_python_app/bin/start")
