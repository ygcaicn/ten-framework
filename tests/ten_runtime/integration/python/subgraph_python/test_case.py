...
def test_subgraph_python():
    ...
    if sys.platform == "win32":
        print("test_subgraph_python doesn't support win32")
        assert False
    elif sys.platform == "darwin":
        # client depends on some libraries in the TEN app.
        my_env["DYLD_LIBRARY_PATH"] = os.path.join(
            base_path, "subgraph_python_app/lib"
        )
    else:
        # client depends on some libraries in the TEN app.
    ...
    if sys.platform == "linux":
        if build_config_args.enable_sanitizer:
            libasan_path = os.path.join(
                base_path,
                (
                    "subgraph_python_app/ten_packages/system/"
    ...
    if not is_started:
        ...
        print("The exit code of subgraph_python: ", exit_code)
