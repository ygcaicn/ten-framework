"""
Test call_api_during_closing_python.
"""
...
def test_call_api_during_closing_python():
    ...
    if sys.platform == "win32":
        ...
        my_env["DYLD_LIBRARY_PATH"] = os.path.join(
            base_path, "call_api_during_closing_python_app/lib"
        )
    else:
        # client depends on some libraries in the TEN app.
    ...
    bootstrap_cmd = os.path.join(
        base_path, "call_api_during_closing_python_app/bin/bootstrap"
    )
    ...
    if sys.platform == "linux":
        if build_config_args.enable_sanitizer:
            libasan_path = os.path.join(
                base_path,
                (
                    "call_api_during_closing_python_app/ten_packages/system/"
    ...
    if not is_started:
        print(
            "The call_api_during_closing_python is not started after 30 seconds."
        )

        server.kill()
        exit_code = server.wait()
        print("The exit code of call_api_during_closing_python: ", exit_code)
