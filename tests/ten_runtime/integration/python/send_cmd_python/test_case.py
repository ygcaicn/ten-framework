...
def test_send_cmd_python():
    ...
    if sys.platform == "win32":
        print("test_send_cmd_python doesn't support win32")
        assert False
    ...
    bootstrap_cmd = os.path.join(base_path, "send_cmd_python_app/bin/bootstrap")
    ...
    if sys.platform == "linux":
        if build_config_args.enable_sanitizer:
            libasan_path = os.path.join(
                base_path,
                (
                    "send_cmd_python_app/ten_packages/system/"
    ...
    server_cmd = os.path.join(base_path, "send_cmd_python_app/bin/start")
    ...
    if not is_started:
        print("The send_cmd_python is not started after 30 seconds.")
    ...
    try:
        ...
        if not is_stopped:
            print("The send_cmd_python can not stop after 30 seconds.")
            server.kill()
