...
def test_env_var_1_python():
    ...
    if sys.platform == "win32":
        print("test_env_var_1_python doesn't support win32")
        assert False

    app_dir_name = "env_var_1_python_app"
    ...
    bootstrap_cmd = os.path.join(
        base_path, "env_var_1_python_app/bin/bootstrap"
    )
    ...
    if sys.platform == "linux":
        if build_config_args.enable_sanitizer:
            libasan_path = os.path.join(
                base_path,
                (
                    "env_var_1_python_app/ten_packages/system/"
    ...
    server_cmd = os.path.join(base_path, "env_var_1_python_app/bin/start")
    ...
    if not is_started:
        print("The env_var_1_python is not started after 30 seconds.")

        server.kill()
        exit_code = server.wait()
        print("The exit code of env_var_1_python: ", exit_code)
    ...
    try:
        ...
        if not is_stopped:
            print("The env_var_1_python can not stop after 30 seconds.")
            server.kill()
