...
def test_send_recv_pcm_python():
    ...
    if sys.platform == "win32":
        print("test_send_recv_pcm_python doesn't support win32")
        assert False
    elif sys.platform == "darwin":
        # client depends on some libraries in the TEN app.
        my_env["DYLD_LIBRARY_PATH"] = os.path.join(
            base_path, "send_recv_pcm_python_app/lib"
        )
    else:
        # client depends on some libraries in the TEN app.
    ...
    app_dir_name = "send_recv_pcm_python_app"
    ...
    if sys.platform == "linux":
        if build_config_args.enable_sanitizer:
            libasan_path = os.path.join(
                base_path,
                (
                    "send_recv_pcm_python_app/ten_packages/system/"
    ...
    if not is_started:
        print("The send_recv_pcm_python is not started after 30 seconds.")

        server.kill()
        exit_code = server.wait()
        print("The exit code of send_recv_pcm_python: ", exit_code)
    ...
    try:
        ...
        if not is_stopped:
            print("The send_recv_pcm_python can not stop after 30 seconds.")
            server.kill()
