"""
Test resp_handler_yield_python.
"""
...
def test_resp_handler_yield_python():
    ...
    if sys.platform == "win32":
        print("test_resp_handler_yield_python doesn't support win32")
        assert False
    elif sys.platform == "darwin":
        # client depends on some libraries in the TEN app.
        my_env["DYLD_LIBRARY_PATH"] = os.path.join(
            base_path, "resp_handler_yield_python_app/lib"
        )
    else:
        # client depends on some libraries in the TEN app.
    ...
    app_dir_name = "resp_handler_yield_python_app"
    ...
    if sys.platform == "linux":
        if build_config_args.enable_sanitizer:
            libasan_path = os.path.join(
                base_path,
                (
                    "resp_handler_yield_python_app/ten_packages/system/"
    ...
    if not is_started:
        ...
        print("The exit code of resp_handler_yield_python: ", exit_code)
    ...
    try:
        ...
        if not is_stopped:
            print(
                "The resp_handler_yield_python can not stop after 30 seconds."
            )
            server.kill()
