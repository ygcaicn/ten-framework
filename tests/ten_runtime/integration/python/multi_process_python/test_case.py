...
def test_multi_process_python():
    ...
    if sys.platform == "win32":
        print("test_multi_process_python doesn't support win32")
        assert False
    elif sys.platform == "darwin":
        # client depends on some libraries in the TEN app.
        my_env["DYLD_LIBRARY_PATH"] = os.path.join(
            base_path, "multi_process_python_app/lib"
        )
    else:
        # client depends on some libraries in the TEN app.
    ...
    app_dir_name = "multi_process_python_app"
    ...
    bootstrap_cmd = os.path.join(
        base_path, "multi_process_python_app/bin/bootstrap"
    )
    ...
    if not is_started:
        print("The multi_process_python is not started after 30 seconds.")

        server.kill()
        exit_code = server.wait()
        print("The exit code of multi_process_python: ", exit_code)
    ...
    try:
        ...
        if not is_stopped:
            print("The multi_process_python can not stop after 30 seconds.")
            server.kill()
