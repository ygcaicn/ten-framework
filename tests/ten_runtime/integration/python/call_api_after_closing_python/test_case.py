"""
Test call_api_after_closing_python.
"""
...
def test_call_api_after_closing_python():
    ...
    if sys.platform == "win32":
        print("test_call_api_after_closing_python doesn't support win32")
        assert False
    elif sys.platform == "darwin":
        # client depends on some libraries in the TEN app.
        my_env["DYLD_LIBRARY_PATH"] = os.path.join(
            base_path, "call_api_after_closing_python_app/lib"
        )
    else:
        # client depends on some libraries in the TEN app.
    ...
    bootstrap_cmd = os.path.join(
        base_path, "call_api_after_closing_python_app/bin/bootstrap"
    )
    ...
    if not is_started:
        ...
        print("The exit code of call_api_after_closing_python: ", exit_code)
