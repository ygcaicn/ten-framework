"""
Test async_get_set_prop_1_python.
"""
...
def test_async_get_set_prop_1_python():
    ...
    if sys.platform == "win32":
        print("test_async_get_set_prop_1_python doesn't support win32")
        assert False
    elif sys.platform == "darwin":
        # client depends on some libraries in the TEN app.
        my_env["DYLD_LIBRARY_PATH"] = os.path.join(
            base_path, "async_get_set_prop_1_python_app/lib"
        )
    else:
        # client depends on some libraries in the TEN app.
    ...
    app_dir_name = "async_get_set_prop_1_python_app"
    ...
    bootstrap_cmd = os.path.join(
        base_path, "async_get_set_prop_1_python_app/bin/bootstrap"
    )
    ...
    server_cmd = os.path.join(
        base_path, "async_get_set_prop_1_python_app/bin/start"
    )
    ...
    if not is_started:
        ...
        print("The exit code of async_get_set_prop_1_python: ", exit_code)
    ...
    try:
        ...
        if not is_stopped:
            print(
                "The async_get_set_prop_1_python can not stop after 30 seconds."
            )
            server.kill()
