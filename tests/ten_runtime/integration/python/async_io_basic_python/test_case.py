...
def test_async_io_basic_python():
    ...
    if sys.platform == "win32":
        print("test_async_io_basic_python doesn't support win32")
        assert False
    elif sys.platform == "darwin":
        # client depends on some libraries in the TEN app.
        my_env["DYLD_LIBRARY_PATH"] = os.path.join(
            base_path, "async_io_basic_python_app/lib"
        )
    else:
        # client depends on some libraries in the TEN app.
    ...
    app_dir_name = "async_io_basic_python_app"
    ...
    if sys.platform == "linux":
        if build_config_args.enable_sanitizer:
            libasan_path = os.path.join(
                base_path,
                (
                    "async_io_basic_python_app/ten_packages/system/"
    ...
    try:
        ...
        if not is_stopped:
            print("The async_io_basic_python can not stop after 30 seconds.")
            server.kill()
