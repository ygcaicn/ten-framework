...
def test_send_data_python():
    ...
    if sys.platform == "win32":
        print("test_send_data_python doesn't support win32")
        assert False
    elif sys.platform == "darwin":
        # client depends on some libraries in the TEN app.
        my_env["DYLD_LIBRARY_PATH"] = os.path.join(
            base_path, "send_data_python_app/lib"
        )
    else:
        # client depends on some libraries in the TEN app.
    ...
    app_dir_name = "send_data_python_app"
    ...
    bootstrap_cmd = os.path.join(
        base_path, "send_data_python_app/bin/bootstrap"
    )
    ...
    if sys.platform == "linux":
        if build_config_args.enable_sanitizer:
            libasan_path = os.path.join(
                base_path,
                (
                    "send_data_python_app/ten_packages/system/"
    ...
    server_cmd = os.path.join(base_path, "send_data_python_app/bin/start")
    ...
    if not is_started:
        ...
        print("The exit code of send_data_python: ", exit_code)
    ...
    try:
        ...
        if not is_stopped:
            print("The send_data_python can not stop after 30 seconds.")
            server.kill()
