...
class test_extension : public ten::extension_t {
 public:
  ...
  void outer_thread_main(ten::ten_env_proxy_t *ten_env_proxy) {
    while (true) {
      if (trigger) {
        ten_env_proxy->notify(extension_on_notify);
 ...
 private:
  friend void extension_on_notify(ten::ten_env_t &ten_env);
...
TEN_CPP_REGISTER_ADDON_AS_EXTENSION(notify_test_normal_func__test_extension,
                                    test_extension);

}  // namespace

TEST(NotifyTest, NormalFunc) {  // NOLINT
  // Start app.
  auto *app_thread =
