 ...
 public:
  ...
  void outer_thread1_main(ten::ten_env_proxy_t *ten_env_proxy) {  // NOLINT
    for (int i = 0; i < 10; ++i) {
      ...
      // We want to simulate that the memory will be freed when it goes out of
      // the scope.
      Holder _(test_data);
  ...
  void on_stop(ten::ten_env_t &ten_env) override {
    ...
    TEN_LOGD("Join outer thread 1 start");
    outer_thread1->join();
    delete outer_thread1;
    TEN_LOGD("Join outer thread 1 end");

    TEN_LOGD("Join outer thread 2 start");
    outer_thread2->join();
    delete outer_thread2;
    TEN_LOGD("Join outer thread 2 end");
...
TEN_CPP_REGISTER_ADDON_AS_EXTENSION(
    two_threads_attempt_to_suspend_2__test_extension_1, test_extension_1);
TEN_CPP_REGISTER_ADDON_AS_EXTENSION(
    two_threads_attempt_to_suspend_2__test_extension_2, test_extension_2);
...
TEST(OuterThreadTest, TwoThreadsAttemptToSuspend2) {  // NOLINT
  ...
  start_graph_cmd->set_graph_from_json(R"({
           "nodes": [{
               ...
               "addon": "two_threads_attempt_to_suspend_2__test_extension_1",
               "app": "msgpack://127.0.0.1:8001/",
               "extension_group": "basic_extension_group"
             },{
               "type": "extension",
               "name": "test_extension_2",
               "addon": "two_threads_attempt_to_suspend_2__test_extension_2",
               "app": "msgpack://127.0.0.1:8001/",
               "extension_group": "basic_extension_group"
             }],
             "connections": [{
