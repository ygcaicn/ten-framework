...
class TestPollyTTS(unittest.TestCase):
    ...
    def test_async_synthesize_speech_timeout(self, mock_session):
        ...
        async def test_async():
            ...
            result = []
            try:
                async for chunk in polly_tts.async_synthesize_speech("Hello world", timeout=0.1):
                    result.append(chunk)
    ...
    def test_async_synthesize_speech_with_retry_success(self, mock_session):
        ...
        async def test_async():
            polly_tts = PollyTTS(self.params)
            result = []
            async for chunk in polly_tts.async_synthesize_speech_with_retry("Hello world"):
                result.append(chunk)
            return result
    ...
    def test_async_synthesize_speech_with_retry_failure(self, mock_session):
        ...
        self.assertEqual(len(result), 0)
        # check retry count
        self.assertEqual(mock_polly.synthesize_speech.call_count, 3)  # initial call + 2 retries
