...
class OpenAIAsrExtensionTester(AsyncExtensionTester):
...
def test_asr_result(patch_asr_client):
    ...
    tester = OpenAIAsrExtensionTester()
    tester.set_test_mode_single("openai_asr_python", json.dumps(property_json))
    err = tester.run()
