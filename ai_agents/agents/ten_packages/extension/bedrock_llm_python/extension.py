...
class BedrockLLMExtension(AsyncLLMBaseExtension):
    ...
    async def _call_nova_model(
        self, input_text: str, image_buffers: List[bytes]
    ) -> None:
        ...
        try:
            ...
            for m in self.memory:
                ...
                messages.append({"role": m["role"], "content": m_content})
        ...
        for event in response.get("stream"):
            if "contentBlockDelta" in event:
                if "text" in event["contentBlockDelta"]["delta"]:
                    ...
                    while True:
                        ...
                        if not first_sentence_sent:
                            ...
                            self.ten_env.log_info(
                                f"First sentence latency: {(time.time() - start_time)*1000}ms"
                            )
                ...
                if "metrics" in event["metadata"]:
                    self.ten_env.log_info(
                        f"Nova model latency: {event['metadata']['metrics']['latencyMs']}ms"
                    )
    ...
    async def on_tools_update(self, ten_env: AsyncTenEnv, tool) -> None:
