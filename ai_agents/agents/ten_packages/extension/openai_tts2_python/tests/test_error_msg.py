...
def test_empty_params_fatal_error():
    ...
    tester.set_test_mode_single(
        "openai_tts2_python", json.dumps(empty_params_config)
    )
...
def test_invalid_api_key_error(MockOpenaiTTSClient):
    ...
    tester.set_test_mode_single(
        "openai_tts2_python", json.dumps(invalid_key_config)
    )
    ...
    assert tester.error_message is not None, "Error message should not be None"
    assert (
        "Incorrect API key" in tester.error_message
    ), "Error message should mention Incorrect API key"
